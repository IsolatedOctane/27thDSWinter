...
class Actor(Model):
    ...
    def forward(self, x):
        ...
        x = F.tanh(self.l3(x)) * self.action_limit  # TODO
        return x
...
class Critic(Model):
    def __init__(self):
        super().__init__()
        self.l1_state = L.Linear(128)
        self.l1_action = L.Linear(128)
        self.l2 = L.Linear(64)
        self.l3 = L.Linear(1)
    ...
    def forward(self, state, action):
        ...
        h_state = self.l1_state(state)
        h_action = self.l1_action(action)
        x = F.relu(h_state+h_action)
        x = F.relu(self.l2(x))
        x = self.l3(x)
        return x
...
class DDPGAgent:
    ...
    def get_action(self, state, noise_scale=0.1):
        ...
        noise = np.random.randn(self.action_size) * noise_scale
        action_with_noise =  np.clip(action + noise, -self.action_limit, self.action_limit)  # TODO
    ...
    def update(self, batch_size=64):
        ...
        next_actions = self.actor_target(next_states) # TODO

        # TODO: target critic으로 target Q-value 계산
        # 힌트: self.critic_target(next_states, next_actions)
        target_q = self.critic_target(next_states,next_actions)  # TODO
        ...
        critic_loss = F.mean_squared_error(current_q, td_target)  # TODO
...
답변: Experience replay: sample efficiency 향상 Target network: 안정적인 학습을 위해서
'''
